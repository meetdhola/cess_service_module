    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 px-4 pointer-events-none"
      ...
      <div className="relative pointer-events-auto max-w-md mx-auto">
        ...
        <div className="cmd-glass rounded-[26px] overflow-hidden relative">
          ...
          <div className="relative grid grid-cols-3 items-stretch px-1.5 py-2">
            ...
            {/* Timer center — label only */}
            <div className="flex flex-col items-center justify-end pb-1.5 pt-2">
              <span className={`text-[9px] font-black tracking-[0.15em] uppercase mt-8 transition-colors ${sc.labelColor}`}>
                {sc.label}
              </span>
            </div>
...
function WorkerNavItem({ item, active, onClick, indicator, indicatorColor='slate' }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group relative flex flex-col items-center justify-center py-1.5 transition-all active:scale-90">
...
function WorkerCompletionModal({ ticket, open, onClose, onSuccess }) {
  ...
  const canSubmit      = validExpense && !!file;

  const reset = () => { setExpense(''); setNote(''); setFile(null); if (fileRef.current) fileRef.current.value=''; };

  const handleSave = async () => {
    if (!file) { alert('Please attach a completion report file.'); return; }
    if (!validExpense) { alert('Please enter a valid expense amount (0 if none).'); return; }
    setSaving(true);
    try {
      const fd = new FormData();
            ...
            {!file ? (
              ...
              <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-3">
                ...
                <button onClick={() => { setFile(null); if (fileRef.current) fileRef.current.value=''; }}
                  className="w-7 h-7 rounded-full bg-white border border-red-200 text-red-500 hover:bg-red-50 flex items-center justify-center flex-shrink-0 text-xs">✕</button>
              </div>
            )}
          </div>
